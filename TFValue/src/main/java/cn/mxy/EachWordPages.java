...
public class EachWordPages {
    public static void main(String[] args) throws Exception {
        ...
        Get get = new Get(Bytes.toBytes(rowkey));//通过rowkey创建get对象，用于搜索
        get.addColumn(cf, qf);  //指定get搜索列
        get.setCheckExistenceOnly(true);
        Result result = table.get(get);//执行数据读取并返回结果对象

        Get get1 = new Get(Bytes.toBytes(rowkey));//通过rowkey创建get对象，用于写入
        get1.setCheckExistenceOnly(true);
        Result result1 = table.get(get1);//执行数据读取并返回结果对象

        Get get2 = new Get(Bytes.toBytes(rowkey));//通过rowkey创建get对象，用于写入
        Result result2 = table.get(get2);//执行数据读取并返回结果对象
        ...
        if(result1.getExists()) {
            ...
            System.out.println("keyword不在TFValue表中");
        }
